...
const features: Feature[] = [
  {
    icon: BadgeCheck,
    title: 'جودة مضمونة',
    description: 'نعتمد أعلى معايير الجودة في التنفيذ ونستخدم أجود الخامات مع ضمان حقيقي على جميع أعمالنا.',
  },
  {
    icon: Award,
    title: 'خبرة واسعة',
    description: 'فريق من المهندسين والفنيين بخبرة تمتد لسنوات في تنفيذ مشاريع متنوعة في جميع أنحاء المملكة.',
  },
  {
    icon: CalendarCheck,
    title: 'التزام بالمواعيد',
    description: 'نلتزم بجدول التنفيذ المتفق عليه بدقة، لنضمن تسليم مشروعك في الوقت المحدد دون تأخير.',
  },
  {
    icon: Wallet,
    title: 'أسعار تنافسية',
    description: 'عروض أسعار شفافة وتنافسية بدون أي تكاليف خفية، لتلائم مختلف الميزانيات واحتياجات المشاريع.',
  },
]

const heroImages = [
  { src: '/images/hero/sandwich-panel-1.jpg', alt: 'هياكل حديدية ومستودعات عازلة', title: 'هياكل حديدية وهناجر', tag: 'مواصفات هندسية' },
  { src: '/images/hero/sandwich-panel-2.jpg', alt: 'تركيب ألواح ساندوتش بانل للمستودعات', title: 'مستودعات ساندوتش بانل', tag: 'عزل حراري PIR' },
  { src: '/images/hero/canopy-1.jpg', alt: 'مستودعات تجارية وصناعية معزولة', title: 'مستودعات تجارية وصناعية', tag: 'تنفيذ شامل' },
  { src: '/images/hero/canopy-2.jpg', alt: 'مظلات وسواتر ومشاريع عامة', title: 'مظلات ومشاريع عامة', tag: 'ضمان معتمد' },
]
...
export default async function HomePage() {
  ...
  // جلب المشاريع الحية من قاعدة البيانات
  ...
  return (
    <main className="overflow-x-hidden bg-white" dir="rtl">
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-[#0e1424] via-[#162035] to-[#0f172a] py-12 text-white sm:py-16 lg:py-20">
        <div className="pointer-events-none absolute -right-24 top-0 h-72 w-72 sm:h-96 sm:w-96 rounded-full bg-[#c5a059]/10 blur-[100px]" />
        <div className="pointer-events-none absolute -left-24 bottom-0 h-72 w-72 sm:h-96 sm:w-96 rounded-full bg-blue-600/10 blur-[100px]" />

        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid items-center gap-10 lg:grid-cols-12 lg:gap-10">
            <div className="text-right lg:col-span-5">
              <span className="inline-flex items-center gap-2 rounded-full border border-[#c5a059]/40 bg-[#c5a059]/10 px-3.5 py-1.5 text-xs font-bold text-[#c5a059] shadow-inner backdrop-blur-md">
                <BadgeCheck className="h-4 w-4 shrink-0" />
                <span>شركة مقاولات معتمدة في المملكة العربية السعودية</span>
              </span>

              <h1 className="mt-5 text-2xl font-black leading-snug sm:text-4xl lg:text-5xl">
              ...
              <p className="mt-4 text-xs leading-relaxed text-gray-300 sm:text-sm lg:text-base">
              ...
              <div className="mt-6 flex flex-wrap items-center gap-3">
                <Link
                  href="/projects"
                  className="inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-l from-[#c5a059] to-[#d9b87a] px-6 py-3 text-xs sm:text-sm font-bold text-[#1a233a] shadow-lg shadow-[#c5a059]/20 transition-all hover:brightness-105"
                >
                ...
                <Link
                  href="/contact"
                  className="inline-flex items-center justify-center rounded-2xl border border-white/20 bg-white/5 px-6 py-3 text-xs sm:text-sm font-semibold text-white backdrop-blur-md transition hover:border-[#c5a059]/50 hover:bg-white/10"
                >
                  طلب عرض سعر
                </Link>
              </div>

              <div className="mt-8 flex flex-wrap items-center gap-4 sm:gap-6 border-t border-white/10 pt-5 text-xs text-gray-400">
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-6">
                <div className="space-y-3 sm:space-y-6">
                  {heroImages.slice(0, 2).map((item, idx) => (
                    <div
                      key={idx}
                      className="group relative aspect-[4/3.3] overflow-hidden rounded-2xl sm:rounded-3xl border border-white/15 bg-gradient-to-b from-white/5 to-transparent shadow-xl backdrop-blur-sm transition-all duration-500 hover:border-[#c5a059]"
                    >
                      <Image
                        src={item.src}
                        alt={item.alt}
                      ...
                      <div className="absolute bottom-2.5 right-2.5 left-2.5 sm:bottom-3 sm:right-3 sm:left-3 text-right">
                        <span className="inline-block rounded bg-[#c5a059] px-2 py-0.5 text-[9px] sm:text-[10px] font-black text-[#1a233a]">
                          {item.tag}
                        </span>
                        <p className="mt-1 line-clamp-1 text-[11px] sm:text-xs font-bold text-white">
                          {item.title}
                        </p>
                      </div>
                    </div>
                ...
                <div className="space-y-3 pt-4 sm:space-y-6 sm:pt-10">
                  {heroImages.slice(2, 4).map((item, idx) => (
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-[#0e1424]/90 via-[#0e1424]/20 to-transparent" />
                      <div className="absolute bottom-2.5 right-2.5 left-2.5 sm:bottom-3 sm:right-3 sm:left-3 text-right">
                        <span className="inline-block rounded bg-white px-2 py-0.5 text-[9px] sm:text-[10px] font-black text-[#1a233a]">
                          {item.tag}
                        </span>
                        <p className="mt-1 line-clamp-1 text-[11px] sm:text-xs font-bold text-white">
                          {item.title}
      ...
      {/* 3. قسم أبرز الأعمال المتجاوب بالكامل */}
      ...
      {/* 4. قسم لماذا نحن */}
      <section className="bg-[#1a233a] py-16 sm:py-20 text-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <SectionHeading
            eyebrow="لماذا نحن؟"
      ...
      <a
        ...
        className="fixed bottom-5 left-5 z-50 inline-flex items-center gap-2 sm:gap-3 rounded-full bg-[#25d366] px-4 py-3 sm:px-5 sm:py-3.5 text-xs sm:text-sm font-bold text-white shadow-xl shadow-[#25d366]/30 transition duration-300 hover:scale-105"
      >
        <span className="relative flex h-2.5 w-2.5 sm:h-3 sm:w-3">
          <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-white opacity-75" />
          <span className="relative inline-flex h-2.5 w-2.5 sm:h-3 sm:w-3 rounded-full bg-white" />
        </span>
