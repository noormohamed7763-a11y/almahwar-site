...
export default function ContactPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    const msg = `طلب استفسار جديد من الموقع:
الاسم: ${formData.name}
الجوال: ${formData.phone}
الخدمة: ${formData.service}
التفاصيل: ${formData.message}`
    window.open(`${siteConfig.whatsapp}?text=${encodeURIComponent(msg)}`, '_blank')
  }
        ...
        <div className="grid gap-12 lg:grid-cols-12">
          ...
          <div className="space-y-6 lg:col-span-5">
            <div className="rounded-2xl bg-[#1a233a] p-8 text-white shadow-xl">
              <h2 className="text-2xl font-bold text-[#c5a059]">معلومات الشركة</h2>
              <div className="mt-8 space-y-6">
                <div className="flex items-center gap-4">
                  <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-white/10 text-[#c5a059]">
                    <Phone className="h-6 w-6" />
                  </div>
                  <div>
                    <span className="block text-xs text-gray-400">الهاتف المباشر</span>
                    <a href={`tel:${siteConfig.phone}`} className="font-bold text-white hover:text-[#c5a059]">
                      {siteConfig.phone}
                    </a>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <a href={`mailto:${siteConfig.email}`} className="font-bold text-white hover:text-[#c5a059]">
                      {siteConfig.email}
                    </a>
                  </div>
                </div>
            ...
            <form onSubmit={handleSubmit} className="mt-8 space-y-6">
              <div>
                ...
                <input
                  ...
                  className="mt-2 w-full rounded-xl border border-gray-200 px-4 py-3 text-sm outline-none focus:border-[#c5a059]"
                />
              </div>
              ...
              <div>
                <label htmlFor="contact-service" className="block text-sm font-bold text-gray-700">نوع الخدمة</label>
                <select
                  id="contact-service"
                  ...
                  className="mt-2 w-full rounded-xl border border-gray-200 px-4 py-3 text-sm outline-none focus:border-[#c5a059]"
                >
                  <option value="سندوتش بانل">سندوتش بانل</option>
                  <option value="مظلات وسواتر">مظلات وسواتر</option>
                  <option value="ترميم">ترميم</option>
                  <option value="بناء عام">بناء عام</option>
                  <option value="دهانات">دهانات</option>
                  <option value="أسقف مستعارة">أسقف مستعارة</option>
                  <option value="بلاط وسيراميك">بلاط وسيراميك</option>
                  <option value="جبس وديكورات">جبس وديكورات</option>
                </select>
              </div>

              <div>
                <label htmlFor="contact-message" className="block text-sm font-bold text-gray-700">تفاصيل الطلب</label>
                <textarea
                  id="contact-message"
                  rows={4}
                  required
                  placeholder="المساحة، الموقع، أو تفاصيل المشروع..."
              ...
              <button
                type="submit"
                className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#c5a059] py-4 font-bold text-[#1a233a] transition hover:bg-[#d9b87a]"
              >
